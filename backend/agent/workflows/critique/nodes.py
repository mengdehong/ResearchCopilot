...
def supporter_review(
    state: CritiqueState, *, llm: BaseChatModel,
) -> dict:
    """蓝方（支持者）独立评审：只看原始产出物，正面评价。"""
    target_content = _get_target_artifacts(state)
    target_wf = state.get("target_workflow", "")

    prompt = load_prompt("critique/prompts", key="supporter_review", variables={
        "target_wf": target_wf,
...
def critic_review(
    ...
    prompt = load_prompt("critique/prompts", key="critic_review", variables={
        "target_wf": target_wf,
        "target_content": target_content,
    })
...
def judge_verdict(
    ...
    result = llm.with_structured_output(JudgeVerdict).invoke([
        SystemMessage(content=load_prompt(
            "critique/prompts", key="judge_verdict",
            variables={"context": ""},
        )["system"]),
