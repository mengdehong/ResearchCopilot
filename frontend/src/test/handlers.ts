...
export const handlers = [
    ...
    http.get('/api/v1/workspaces/:id/summary', ({ params }) =>
    ...
    http.delete('/api/v1/workspaces/:id', () => HttpResponse.json({ ok: true })),
    ...
    http.post('/api/v1/documents/:id/retry', ({ params }) =>
    ...
    // ── Agent / Threads ──
    http.get('/api/v1/agent/threads', () =>
        HttpResponse.json([
            { thread_id: 'th-1', title: 'Thread 1', status: 'active', updated_at: '2025-01-01', workspace_id: 'ws-1' },
        ]),
    ),
    http.post('/api/v1/agent/threads', () =>
        HttpResponse.json({ thread_id: 'th-new', title: 'New Thread', status: 'active', workspace_id: 'ws-1' }),
    ),
    http.post('/api/v1/agent/threads/:threadId/runs', () =>
        HttpResponse.json({ run_id: 'run-1', thread_id: 'th-1', status: 'running', stream_url: '/api/v1/agent/threads/th-1/runs/run-1/stream' }),
    ),
    http.post('/api/v1/agent/threads/:threadId/runs/:runId/resume', () =>
        HttpResponse.json({ run_id: 'run-1', status: 'running' }),
    ),
    http.post('/api/v1/agent/threads/:threadId/runs/:runId/cancel', () =>
        HttpResponse.json({ ok: true }),
    ),
    http.delete('/api/v1/agent/threads/:threadId', () => HttpResponse.json({ ok: true })),

    // ── Quota ──
    http.get('/api/v1/quota/status', () =>
        HttpResponse.json({ total_used: 1000, total_limit: 10000, remaining: 9000, usage_percent: 10, workspaces: [] }),
    ),

    // ── Editor / Draft ──
    http.get('/api/v1/editor/draft/:threadId', ({ params }) =>
        HttpResponse.json({ thread_id: params.threadId, content: '# Draft', updated_at: '2025-01-01' }),
    ),
    http.put('/api/v1/editor/draft', () =>
