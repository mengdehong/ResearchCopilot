...
| **内部流程** | 组装汇报大纲 → LLM 生成结构化 Markdown → `interrupt()` 请求用户确认定稿（approve→继续 / reject→推送至 Canvas 编辑器由用户手改，改完确认后回流）→ python-pptx 渲染 PPTX → 插入文献引用角标 → 打包 ZIP（原始文献+报告+代码+图表+PPT） |
| **输出**     | Markdown 报告 + PPTX + 可下载 ZIP 归档包                                                                                                                                                                                            |
| **特点**     | 只交付生产力资产，精细排版留给本地 Office；用户可在 Canvas 编辑器中精修 Markdown 后再触发渲染                                                                                                                                       |
